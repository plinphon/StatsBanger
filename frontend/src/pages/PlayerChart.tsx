...
export default function PlayerChart() {
  ...
  const handleNextMatch = () => {
    if (currentMatchIndex < recentMatches.length - 1) {
    ...
    <div className="flex justify-between items-center mb-4">
      <button
        ...
        className={`px-4 py-2 rounded ${
          currentMatchIndex === 0 ? "bg-gray-300 cursor-not-allowed" : "bg-blue-500 text-white"
        }`}
      >
        &lt; Previous
      </button>
      <h4 className="text-lg font-bold text-gray-700">
        Match {currentMatchIndex + 1} of {recentMatches.length}
      </h4>
      <button
        onClick={handleNextMatch}
        disabled={currentMatchIndex === recentMatches.length - 1}
        className={`px-4 py-2 rounded ${
          currentMatchIndex === recentMatches.length - 1
...
{isDNP(recentMatches[currentMatchIndex]) ? (
  ...
  <table className="text-xs text-gray-500 border-collapse border border-gray-300 w-full">
    ...
    <tbody>
      {Object.entries(recentMatches[currentMatchIndex].match_stats)
